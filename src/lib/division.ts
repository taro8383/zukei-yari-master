...
function generateDivisionWithRemainderQuestions(): DivisionQuestion[] {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    questions.push({
      ...
      text: [
        `${dividend}個のクッキーを${divisor}人に同じ数ずつ分けます。1人に何個ずつ分けられて、何個あまりますか？`,
        `${dividend}枚のシールを${divisor}人に同じ枚数ずつ配ります。1人何枚もらえて、何枚あまりますか？`,
        `${dividend}個のみかんを${divisor}つのかごに同じ数ずつ入れます。1かごに何個入って、何個あまりますか？`,
        `けいくんは${dividend}枚のカードを${divisor}人の友だちに同じ枚数ずつあげます。1人に何枚あげられて、何枚あまりますか？`,
        `${dividend}このチョコレートを${divisor}人で同じ数ずつ分けます。1人何個もらえて、何個あまりますか？`,
        `${dividend}本のえんぴつを${divisor}つのペンケースに同じ数ずつ入れます。1つに何本入って、何本あまりますか？`,
      ][i % 6],
      textEn: [
        `Kei has ${dividend} cookies to share equally among ${divisor} friends. How many does each get, and how many are left over?`,
        `There are ${dividend} stickers to give equally to ${divisor} people. How many each, with how many remaining?`,
        `${dividend} oranges are packed equally into ${divisor} baskets. How many per basket, and how many are left?`,
        `Kei wants to give ${dividend} cards equally to ${divisor} friends. How many cards each, with how many left over?`,
        `${dividend} chocolates are divided equally among ${divisor} people. How many each, with how many remaining?`,
        `${dividend} pencils are packed equally into ${divisor} pencil cases. How many per case, with how many left?`,
      ][i % 6],
      dividend,
      divisor,
      quotient,
...
function generateLongDivisionQuestions(): DivisionQuestion[] {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    questions.push({
      ...
      topic: 'long-division',
      text: [
        `けいくんのクラスでは${dividend}枚の折り紙を${divisor}人に同じ枚数ずつ配ります。1人は何枚もらえますか？`,
        `${dividend}円を${divisor}日に同じ金額ずつ使います。1日は何円使えますか？`,
        `${dividend}個のりんごを${divisor}つのバスケットに同じ数ずつ入れます。1つのバスケットには何個入りますか？`,
        `${dividend}まいの画用紙を${divisor}グループに同じ枚数ずつ分けます。1グループは何まいもらえますか？`,
        `${dividend}このビーズを${divisor}本のひもに同じ数ずつつけます。1本に何個つきますか？`,
        `工場で${dividend}このお菓子を${divisor}つの箱に同じ数ずつつめます。1箱に何個入りますか？`,
      ][i % 6],
      textEn: [
        `Kei's class shares ${dividend} sheets of origami paper equally among ${divisor} students. How many does each student get?`,
        `${dividend} yen is spent equally over ${divisor} days. How much is spent each day?`,
        `${dividend} apples are placed equally into ${divisor} baskets. How many apples go in each basket?`,
        `${dividend} sheets of drawing paper are divided equally among ${divisor} groups. How many sheets does each group get?`,
        `${dividend} beads are strung equally onto ${divisor} strings. How many beads go on each string?`,
        `A factory packs ${dividend} snacks equally into ${divisor} boxes. How many snacks go in each box?`,
      ][i % 6],
      dividend,
      divisor,
      quotient,
...
function generateDivisionPropertiesQuestions(): DivisionQuestion[] {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    questions.push({
      ...
      text: [
        `${pair.dividend}このお菓子を${pair.divisor}人に同じ数ずつ分けます。わり算の性質を使って計算しましょう。`,
        `${pair.dividend}円を${pair.divisor}人で同じ金額ずつ分けます。わり算の性質を使うとかんたんに計算できます。`,
        `けいくんのクラスでは${pair.dividend}枚のシールを${pair.divisor}人に配ります。性質を使って求めましょう。`,
        `${pair.dividend}mのひもを${pair.divisor}本に等しく切ります。わり算の性質を使って計算しましょう。`,
        `${pair.dividend}個のブロックを${pair.divisor}セットにわけます。わり算の性質を使うとラクに計算できますよ。`,
      ][i % 5],
      textEn: [
        `${pair.dividend} snacks are shared equally among ${pair.divisor} people. Use the properties of division to calculate.`,
        `${pair.dividend} yen is divided equally among ${pair.divisor} people. Use division properties to simplify the calculation.`,
        `Kei's class distributes ${pair.dividend} stickers among ${pair.divisor} students. Use division properties to find the answer.`,
        `A ${pair.dividend}m rope is cut into ${pair.divisor} equal pieces. Use division properties to calculate.`,
        `${pair.dividend} blocks are split into ${pair.divisor} equal sets. Division properties make this easier!`,
      ][i % 5],
...
function generateLongDivision2DigitQuestions(): DivisionQuestion[] {
  ...
  selected.forEach(({ dividend, divisor, quotient }, index) => {
    questions.push({
      ...
      text: [
        `学校の図書館に${dividend}冊の本があります。${divisor}クラスに同じ冊数ずつ配ります。1クラスは何冊もらえますか？`,
        `${dividend}このブロックを${divisor}セットに同じ数ずつ分けます。1セットには何個入りますか？`,
        `けいくんたちは${dividend}mのロープを${divisor}本に同じ長さずつ切ります。1本は何mになりますか？`,
        `工場で${dividend}このおもちゃを${divisor}つの箱につめます。1箱に何個入りますか？`,
        `${dividend}円のプレゼントを${divisor}人で同じ金額ずつ出し合って買います。1人は何円出しますか？`,
        `農場で${dividend}個のたまごを${divisor}パックに同じ数ずつ入れます。1パックは何個入りですか？`,
      ][index % 6],
      textEn: [
        `The school library has ${dividend} books to distribute equally among ${divisor} classes. How many books per class?`,
        `${dividend} building blocks are sorted equally into ${divisor} sets. How many blocks are in each set?`,
        `Kei's group cuts ${dividend}m of rope into ${divisor} equal pieces. How long is each piece?`,
        `A factory packs ${dividend} toys equally into ${divisor} boxes. How many toys per box?`,
        `A ${dividend}-yen gift is paid for equally by ${divisor} people. How much does each person pay?`,
        `A farm packs ${dividend} eggs equally into ${divisor} cartons. How many eggs per carton?`,
      ][index % 6],
      dividend,
      divisor,
      quotient,
...
function generateMentalDivisionQuestions(): DivisionQuestion[] {
  ...
  selected.forEach(({ dividend, divisor, quotient, simplifiedDividend, simplifiedDivisor }, index) => {
    questions.push({
      ...
      text: [
        `${dividend}円を${divisor}人で同じ金額ずつ分けます。0を消す裏ワザを使って暗算しましょう。`,
        `けいくんのチームは${dividend}このシールを${divisor}人で分けます。頭の中で計算しましょう。`,
        `${dividend}mのロープを${divisor}本に等しく切ります。0を消してかんたんに計算しましょう。`,
        `工場で${dividend}個の商品を${divisor}台のトラックに同じ数ずつのせます。暗算でもとめましょう。`,
        `${dividend}円のお金を${divisor}日に分けて使います。1日分は何円ですか？頭の中で計算しよう。`,
      ][index % 5],
      textEn: [
        `${dividend} yen is shared equally among ${divisor} people. Use the zero-canceling trick to calculate mentally.`,
        `Kei's team shares ${dividend} stickers among ${divisor} people. Calculate it in your head!`,
        `A ${dividend}m rope is cut into ${divisor} equal pieces. Cancel the zeros for an easy mental calculation.`,
        `A factory loads ${dividend} items equally onto ${divisor} trucks. Find the answer mentally.`,
        `${dividend} yen is spread equally over ${divisor} days. How much per day? Calculate mentally.`,
      ][index % 5],
      dividend,
      divisor,
      quotient,
