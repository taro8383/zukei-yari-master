...
export function generateFractionTypeQuestions(): FractionQuestion[] {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    questions.push({
      ...
      fractionType: 'proper',
      text: `ピザを${den}等分して${num}切れ食べました。「${formatFraction(num, den)}」はどの種類の分数ですか？`,
      textEn: `A pizza cut into ${den} slices — you ate ${num}. What type of fraction is "${formatFraction(num, den)}"?`,
  ...
  for (let i = 0; i < 2; i++) {
    ...
    questions.push({
      ...
      text: `けいくんはリボンを${den}等分したものを${num}本持っています。「${formatFraction(num, den)}」はどの種類の分数ですか？`,
      textEn: `Kei has ${num} pieces of ribbon, each 1/${den} of a meter. What type of fraction is "${formatFraction(num, den)}"?`,
  ...
  questions.push({
    ...
    fractionType: 'mixed',
    text: `ケーキを${den}等分したものが${whole}個と${num}切れあります。「${formatMixedNumber(whole, num, den)}」はどの種類ですか？`,
    textEn: `There are ${whole} whole cakes and ${num} out of ${den} slices. What type of fraction is "${whole} ${num}/${den}"?`,
...
export function generateConvertingQuestions(): FractionQuestion[] {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    questions.push({
      ...
      text: [
        `リボンが${formatFraction(num, den)}mあります。帯分数に直しましょう。`,
        `ジュースが${formatFraction(num, den)}Lあります。帯分数に直しましょう。`,
        `けいくんはロープを${formatFraction(num, den)}m持っています。帯分数に直しましょう。`,
      ][i % 3],
      textEn: [
        `There is ${formatFraction(num, den)}m of ribbon. Convert to a mixed number.`,
        `There is ${formatFraction(num, den)}L of juice. Convert to a mixed number.`,
        `Kei has ${formatFraction(num, den)}m of rope. Convert to a mixed number.`,
      ][i % 3],
  ...
  for (let i = 0; i < 2; i++) {
    ...
    questions.push({
      ...
      answerDen: den,
      text: [
        `ジュースが${formatMixedNumber(whole, num, den)}Lあります。仮分数に直しましょう。`,
        `けいくんはリボンを${formatMixedNumber(whole, num, den)}m持っています。仮分数に直しましょう。`,
      ][i % 2],
      textEn: [
        `There is ${whole} and ${num}/${den}L of juice. Convert to an improper fraction.`,
        `Kei has ${whole} and ${num}/${den}m of ribbon. Convert to an improper fraction.`,
      ][i % 2],
...
export function generateAddingFractionsQuestions(): FractionQuestion[] {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    questions.push({
      ...
      text: [
        `ピザを${formatFraction(num1, den)}切れ食べて、あとで${formatFraction(num2, den)}切れ食べました。合わせて何切れ分食べましたか？`,
        `ジュースが${formatFraction(num1, den)}Lと${formatFraction(num2, den)}Lあります。合わせて何Lですか？`,
        `けいくんはリボンを${formatFraction(num1, den)}mと${formatFraction(num2, den)}m持っています。合わせて何mありますか？`,
        `ケーキを${formatFraction(num1, den)}食べて、妹も${formatFraction(num2, den)}食べました。2人合わせて何個分食べましたか？`,
        `牛乳が${formatFraction(num1, den)}Lと${formatFraction(num2, den)}Lあります。全部で何Lありますか？`,
      ][i % 5],
      textEn: [
        `You ate ${formatFraction(num1, den)} of a pizza, then ${formatFraction(num2, den)} more. How much pizza did you eat in total?`,
        `There is ${formatFraction(num1, den)}L of juice and ${formatFraction(num2, den)}L more. How many liters in total?`,
        `Kei has ${formatFraction(num1, den)}m of ribbon and ${formatFraction(num2, den)}m more. How many meters altogether?`,
        `You ate ${formatFraction(num1, den)} of a cake and your sister ate ${formatFraction(num2, den)} too. How much cake was eaten together?`,
        `There is ${formatFraction(num1, den)}L of milk and ${formatFraction(num2, den)}L more. How many liters altogether?`,
      ][i % 5],
      explanation,
      explanationEn,
    });
...
export function generateSubtractingFractionsQuestions(): FractionQuestion[] {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    questions.push({
      ...
      text: [
        `けいくんは${formatMixedNumber(whole1, num1, den)}mのリボンを持っています。${formatFraction(num2, den)}m使いました。残りは何mですか？`,
        `ジュースが${formatMixedNumber(whole1, num1, den)}Lありました。${formatFraction(num2, den)}L飲みました。何L残っていますか？`,
        `けいくんはロープを${formatMixedNumber(whole1, num1, den)}m持っています。${formatFraction(num2, den)}m切り取りました。残りは何mですか？`,
        `水が${formatMixedNumber(whole1, num1, den)}Lありました。${formatFraction(num2, den)}L使いました。残りは何Lですか？`,
        `ケーキが${formatMixedNumber(whole1, num1, den)}切れありました。${formatFraction(num2, den)}切れ食べました。残りは何切れですか？`,
      ][i % 5],
      textEn: [
        `Kei has ${whole1} and ${num1}/${den}m of ribbon. He used ${formatFraction(num2, den)}m. How much remains?`,
        `There was ${whole1} and ${num1}/${den}L of juice. You drank ${formatFraction(num2, den)}L. How much is left?`,
        `Kei has ${whole1} and ${num1}/${den}m of rope. He cut off ${formatFraction(num2, den)}m. How much rope remains?`,
        `There was ${whole1} and ${num1}/${den}L of water. ${formatFraction(num2, den)}L was used. How much water is left?`,
        `There were ${whole1} and ${num1}/${den} slices of cake. ${formatFraction(num2, den)} slices were eaten. How many slices remain?`,
      ][i % 5],
      explanation,
      explanationEn,
    });
